...
String ESPWebFileManager::sanitizePath(const String & path) {
  ...
  if (sanitized.indexOf("/../") >= 0 || sanitized.endsWith("/..") || sanitized == "/..") {
    return "";
  }
  return sanitized;
}
...
void ESPWebFileManager::listDir(const char * dirname, uint8_t levels) {
  ...
  while (file) {
    ...
    if (file.isDirectory()) {
      str_data += "1," + entryName + ",-"; // Folders don't have sizes
    } else {
      str_data += "0," + entryName + "," + String(file.size());
    }
...
void ESPWebFileManager::setServer(AsyncWebServer * server) {
  ...
  server -> on("/get-folder-contents", HTTP_GET, [ & ](AsyncWebServerRequest * request) {
    ...
    String path = sanitizePath(request -> arg("path"));
    if (path.isEmpty()) {
  ...
  server -> on("/upload", HTTP_POST, [ & ](AsyncWebServerRequest * request) {
    ...
    String file_path = dir_path == "/" ? "/" + filename : dir_path + "/" + filename;

    if (!index) {
      if (dir_path != "/" && !current_fs -> exists(dir_path)) {
